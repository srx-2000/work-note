...
1. 在进入一个函数的时候，`x0`到`x7`是寄存器分别存储了传入的8个参数
2. 在退出一个函数的时候，通常会以`x0`寄存器作为返回值的存储寄存器。但当返回的内容大于16字节时，会将返回值放到`x8`寄存器中存储。
3. fp【Frame Pointer Register】是x29，lr【link register】是x30。具体作用及在内存中的管理，详见[关键概念理解](#关键概念理解)中的第二个。

## 关键概念理解

1. **栈顶指针和栈底指针**

   ​	首先要明确的一件事是：在arm和x86汇编中，**栈是倒着的**，如果类比的话。可以把这个栈类比成一个倒着的瓶子，栈底指针就是瓶子底，栈顶指针就是瓶子里不受重力影响的水面。这样stp这种开栈操作就像是给瓶子里加水，栈顶指针就会下移，从而离栈底越来越远。而ldp这种指令则像是从瓶子里取水，栈顶指针会上移，离栈底越来越近。这里的上移与下移就是内存中栈顶指针地址的变化。

2. **x29和x30【fp和lr】**

   ​	通用寄存器中两个特殊寄存器：x29和x30。

   ​	他们的作用分别是：栈帧寄存器fp和链接寄存器lr，fp保存的是当前函数的起始地址，lr保存的则是该函数的返回地址。

   ​	当进入到一个函数的时候会优先保存这两个寄存器。

   > 这两个寄存器的赋值位置是不同的：
   >
   > ​	lr寄存器的赋值位置是当前要调用的函数还没有调用时，此时在lr中保存这个函数调用指令的下一条指令的地址。
   >
   > ​	fp寄存器赋值的位置则是当前要调用的函数的内部，在已经使用stp保存了x29和x30之后【**此时x29和x30内存储的分别是上一个函数的fp和当前函数的lr**】，此时会将sp的值赋值给x29，覆盖掉上一个函数的fp，而后我们就可以使用新赋值的这个x29这个寄存器中的值访问当前函数中的局部变量了。
   >
   > ​	至于fp怎么使用：就直接fp加减不超过当前函数地址范围的相对偏移即可
   >
   > 
